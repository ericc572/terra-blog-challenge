...
pub fn execute(
    ...
    match msg {
        ExecuteMsg::Create { 
            title,
            text,
        } => try_create_post(deps, text, title),
    }
}

pub fn try_create_post(deps: DepsMut, text: String, title: String) -> Result<Response, ContractError> {
    POST.update(deps.storage, |mut post| -> Result<_, ContractError> {
        post.title = title;
        post.text = text;
        Ok(post)
    })?;
...
fn query_post(deps: Deps) -> StdResult<PostResponse> {
    let post = POST.load(deps.storage)?;
    Ok(PostResponse { title: post.title, text: post.text })
}
